...
export default function Header() {
  ...
  const [musicClassesDropdownOpen, setMusicClassesDropdownOpen] = useState(false);
  ...
  const musicClasses = [
    ...
    { href: "/music-classes/guitar-classes-at-home", label: "Guitar Classes at Home" },
    { href: "/music-classes/singing-classes-at-home", label: "Singing Classes at Home" },
    { href: "/music-classes/piano-classes-at-home", label: "Piano Classes at Home" },
  ];
  ...
  return (
    //Commented code is 
    // <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${isMounted && scrollY > 50 ? 'bg-black/90 backdrop-blur-xl py-0.5 sm:py-1' : 'bg-transparent py-1 sm:py-1.5'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="group flex items-center gap-2 sm:gap-3">
            <div className="w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 xl:w-28 xl:h-28">
              <Image
                src="/mz-logo.png"
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                onClick={() => setMusicClassesDropdownOpen(!musicClassesDropdownOpen)}
                className="text-white/80 hover:text-white text-sm font-medium transition-colors relative group flex items-center gap-1"
              >
                Music Classes
                <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-red-500 transition-all group-hover:w-full"></span>
                <svg
                  className={`w-4 h-4 transition-transform ${musicClassesDropdownOpen ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
    ...
    setScrollY(100); 
        ...
        {/* Mobile Menu */}
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${mobileMenuOpen ? 'max-h-[600px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
